...
export class SendMaterialComponent {
    ...
    private getItemFromMMSID(mmsid) {
        ...
        return itemOrError.pipe(catchError(() => of('Warning: Input is not an MMSID')));
    }
    ...
    private getBibPostFromMMSID(mmsid: string) {
        return this.almaService.getBibPostFromMMSID(mmsid).pipe(
            catchError(()=> {
                throw new Error(`Warning: Input is not an MMSID`);
            })
        )
    }
    ...
    send() {
        ...
        if (inputBox && !this.isSending) {
            ...
            const result = itemOrError.pipe(
                ...
                concatMap(item => item === 'Warning: Input is not an MMSID' ? this.sendItem(inputBox) : this.sendRelatedItem(item))
            )
